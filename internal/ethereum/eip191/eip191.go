...
// SignPersonalMessage signs a personal message using the provided private key following EIP-191 standard
func SignPersonalMessage(privateKey *ecdsa.PrivateKey, message []byte) (string, error) {
	hashedMessage := SignHash(message)

	sig, err := crypto.Sign(hashedMessage, privateKey)
	if err != nil {
		return "", err
	}
